...
from MainWindow2 import Window
...
class Ui_MainWindow(object):
    ...
    def remCell(self,command = False, keytype = ""):#DONETODO add delete key jit
        ...
        if self.layoutchildren[self.activeid].cursorPosition()==0 and (self.activeid+1<len(self.layoutchildren) or self.activeid-1>=0) and keytype=="backspace":
        ...
        elif self.layoutchildren[self.activeid].cursorPosition()==len(self.layoutchildren[self.activeid].displayText()) and (self.activeid+1<len(self.layoutchildren) or self.activeid-1>=0) and keytype=="delete":
    ...
    def textCheck(self):
        ...
        if check == "" and self.activeid + 1 < len(self.layoutchildren) and self.layoutchildren[self.activeid + 1].displayText().startswith("output::::"):
            self.remCell(True)
        ...
        if m:
            ...
            if command in linecommands.commandslist:
                if not (self.activeid + 1 < len(self.layoutchildren) and self.layoutchildren[self.activeid + 1].displayText().startswith("output::::")):
                    self.newCell(True)
                ...
                self.layoutchildren[self.activeid + 1].setText("output::::" + str(output))



    def cursorKey(self, direction):
        cursor = self.layoutchildren[self.activeid].cursorPosition()
        movement = direction*2  if 0 <= self.activeid+2*direction < len(self.layoutchildren) and self.layoutchildren[self.activeid+direction].isReadOnly() else direction
    ...
    def initialize(self, index):
        ...
        self.lines[index].textEdited.connect(self.textCheck)
